...
// src/core/preview/substitute.ts
var CAPTURE_REF_RE = /\$(?:&|\$|<([^>]+)>|(\d+))/g;
var MACRO_RE = /\{\{([^{}]+?)\}\}/g;
function substituteForPreview(template, ctx = {}) {
  if (!template)
    return template;
  ...
  let out = template.replace(CAPTURE_REF_RE, (match, namedGroup, numberedGroup) => {
    if (match === "$&" || match === "$$")
      return match;
    if (typeof namedGroup === "string")
      return `‹$${namedGroup}›`;
    if (typeof numberedGroup === "string")
      return `‹$${numberedGroup}›`;
    return match;
  });
  out = out.replace(MACRO_RE, (match, body) => {
...
// src/ui/portal-tab.ts
var HIGH_YES_LABEL = "high confidence";
var AMBIGUOUS_LABEL = "mid confidence";
var HIGH_NO_LABEL = "low confidence";
function mountPersistentUisPanel(opts) {
  const { sendToBackend, log } = opts;
  ...
  function pendingKey(characterId, candidateId) {
  ...
  function requestCandidates(characterId) {
    setStatus("Loading…");
    sendToBackend({ type: "request_portal_candidates", characterId });
  }
  function renderForSelectedCard() {
    sectionsHost.innerHTML = "";
    if (selectedCardId === null) {
    ...
    setStatus(`"${characterName}" — ${total} candidate(s): ` + `${highYes.length} high · ${ambiguous.length} mid · ${highNo.length} low`);
    if (highYes.length > 0) {
  ...
  function renderCandidate(c, snap) {
    ...
    if (c.source.kind === "regex_rule") {
      ...
      sourceLabel.textContent = c.source.alt_index === 0 ? "Greeting (first message)" : `Greeting (alternate ${c.source.alt_index})`;
    }
    head.appendChild(sourceLabel);
    const confBadge = document.createElement("span");
    confBadge.className = `lrp-conf lrp-conf-${c.confidence}`;
    confBadge.textContent = c.confidence === "high-yes" ? HIGH_YES_LABEL : c.confidence === "ambiguous" ? AMBIGUOUS_LABEL : HIGH_NO_LABEL;
    ...
    if (c.triggering_selectors.length > 0) {
      ...
      const sourceText = c.triggering_css_source === "both" ? "bg-html + rule's own <style>" : c.triggering_css_source === "rule_inline_style" ? "rule's own <style>" : c.triggering_css_source === "bg_html" ? "bg-html" : 'inline style="…" attribute';
      hint.textContent = `Matched ${c.triggering_selectors.length} selector(s) in ${sourceText}: `;
      const code = document.createElement("code");
      code.textContent = c.triggering_selectors.slice(0, 4).join(", ") + (c.triggering_selectors.length > 4 ? `, +${c.triggering_selectors.length - 4} more` : "");
    ...
    previewDetails.addEventListener("toggle", () => {
      if (!previewDetails.open || mounted)
        return;
  ...
  function mountShadowPreview(host, c, _snap, characterName) {
    ...
    let root2;
    try {
      root2 = shadowHost.attachShadow({ mode: "open" });
    } catch (err) {
  ...
  function handleBackendMessage(msg) {
    if (msg.type === "cards_updated") {
      ...
      for (const key of [...pendingDecisions]) {
        if (key.startsWith(`${msg.characterId}|`))
...
`).length + 2));
      ...
      clearBtn.addEventListener("click", () => {
        if (!window.confirm("Clear background HTML?"))
          return;
        editingBackgroundHtmlBuffer = "";
        commitBackgroundHtmlEdit();
      });
  ...
  function commitBackgroundHtmlEdit() {
  ...
  async function onAddAssetClicked() {
    ...
    try {
      ...
      assetUploadStatus = { kind: "error", message: `Upload failed: ${errMsg2(err)}` };
      render();
      return;
    }
...
`;
          const islandPortalHide = `[data-risu-portal] { display: none !important; }
`;
          const islandCss = stripCssImports(islandLineHeight + islandImgReset + islandPortalHide + islandBundle.css);
...
`;
        const portalHide = `[data-message-id] [data-risu-portal] { display: none !important; }
`;
...
` : "") + lineHeight + imgReset + portalHide + bubbleContainment + rest;
        upsertChatScopeStyle(chatScopeCss);
        flog.info(`bg-html renderer: chat-scope CSS injected css_len=${chatScopeCss.length} ` + `(imports_hoisted_len=${imports.length}, body_len=${rest.length}; ` + `+img-reset +portal-hide +bubble-containment preambles)`);
      }
...
// src/portal/sidebar-portal.ts
var PORTAL_ROOT_ID = "risu-compat-sidebar-portal-root";
var PORTAL_WRAPPER_CLASS = "risu-compat-portal-wrapper";
function setupSidebarPortal(flog) {
  const portalRoot = ensurePortalRoot();
  const mounted = new Map;
  let activeChatId = null;
  let lastSeq = 0;
  function clearAll() {
    if (mounted.size === 0 && activeChatId === null)
      return;
  ...
  function applySetPortals(msg) {
    if (msg.chatId !== activeChatId) {
      flog.info(`sidebar-portal: chat-switch ${activeChatId ?? "<none>"} → ${msg.chatId} ` + `(droppingMounted=${mounted.size}, incoming=${msg.portals.length})`);
      for (const [, slot] of mounted)
        slot.wrapper.remove();
      mounted.clear();
      activeChatId = msg.chatId;
    ...
    if (msg.seq <= lastSeq) {
      flog.warn(`sidebar-portal: ignoring stale set_portals seq=${msg.seq} lastSeq=${lastSeq} chatId=${msg.chatId}`);
      return;
    }
    lastSeq = msg.seq;
    ...
    flog.info(`sidebar-portal: applied chatId=${msg.chatId} seq=${msg.seq} ` + `incoming=${msg.portals.length} added=${added} replaced=${replaced} ` + `unchanged=${unchanged} removed=${removed} total_mounted=${mounted.size}`);
  }
