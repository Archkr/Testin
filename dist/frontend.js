...
function setupMessagePortal(ctx, flog2) {
  ...
  function sweep(reason) {
    ...
    for (const containerEl of containers) {
      let visit = function(node) {
        ...
        for (const child of Array.from(childrenList)) {
          ...
          if (!(child instanceof HTMLElement))
            continue;
          if (child.closest(`.${PORTAL_WRAPPER_CLASS}`))
            continue;
          if (child.hasAttribute("popover") || child.tagName === "DIALOG")
            continue;
          let isFixed = false;
          try {
            isFixed = window.getComputedStyle(child).position === "fixed";
          } catch {}
          if (isFixed) {
            if (hasCbAncestorBetween(child, container))
              continue;
            fixed.push(child);
            continue;
          }
      ...
      if (container.closest(`.${PORTAL_WRAPPER_CLASS}`))
        continue;
...
function expandLiftSet(parent, fixedDirectChildren) {
  ...
  const fixedSet = new Set(fixedDirectChildren);
  let start = -1;
  let end = -1;
...
function hasCbAncestorBetween(el, bubble) {
  ...
  while (cur) {
    ...
    if (wc && /\b(?:transform|filter|perspective)\b/.test(wc))
      return true;
    cur = cur.parentNode;
  }
  return false;
