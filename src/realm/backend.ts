...
export interface RealmBackendDeps {
  readonly send: (msg: RealmBackendToFrontend) => void;
  readonly log: RealmBackendLog;
  readonly importCardFromBytes: (bytesB64: string, fileName: string) => Promise<void>;
}

export interface RealmBackendHandle {
  handle(msg: RealmFrontendToBackend): Promise<void>;
  importAnyFormat(bytesB64: string, fileName: string): Promise<void>;
}
...
export function setupRealmBackend(deps: RealmBackendDeps): RealmBackendHandle {
  ...
  async function handle(msg: RealmFrontendToBackend): Promise<void> {
    switch (msg.type) {
      case 'realm_search': {
        log.info(
      ...
      case 'realm_info': {
        ...
        try {
          ...
          send({ type: 'realm_info_result', requestId: msg.requestId, ok: true, info });
        } catch (err) {
          const error = errMessage(err);
          log.warn(`realm_info failed req=${msg.requestId}: ${error}`);
          send({ type: 'realm_info_result', requestId: msg.requestId, ok: false, error });
        }
        break;
      }
      case 'realm_download': {
        ...
        try {
          ...
          await importCardFromBytes(bytesB64, conv.fileName);
        } catch (err) {
          const error = errMessage(err);
  ...
  async function importAnyFormat(bytesB64: string, fileName: string): Promise<void> {
    ...
    if (!conv.synthesized) {
      await importCardFromBytes(bytesB64, fileName);
      return;
    }
    log.info(
    ...
    await importCardFromBytes(convB64, conv.fileName);
  }
